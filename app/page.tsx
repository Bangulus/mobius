...
export default function Page() {
  ...
  const [view, setView] = useState<
    'markets' | 'portfolio' | 'leaderboard' | 'admin' | 'profile'
  >('markets');
  ...
  async function loadMarkets() {
    const response = await fetch(`${supabaseUrl}/rest/v1/markets?select=*`, {
  ...
  async function loadTrending() {
    ...
    const marketsResponse = await fetch(
      `${supabaseUrl}/rest/v1/markets?id=in.(${sortedIds.join(
        ','
  ...
  async function loadTrades(uid: string) {
    ...
    setTrades(
      tradesData.map((trade: any) => ({
        ...trade,
        market: marketsMap[trade.market_id] || null,
      }))
    );
  }
  ...
  async function signIn() {
    ...
    if (data.access_token) {
      setTimeout(() => {
        window.location.href = `/?token=${data.access_token}&user_id=${data.user.id}`;
      }, 2000);
  ...
  if (loggedIn) {
    ...
    const categories = [
      'Trends',
      ...(Array.from(
        new Set(openMarkets.map((m: any) => m.category).filter(Boolean))
      ) as string[]),
    ];
    const filteredMarkets =
      activeCategory === 'Trends'
        ? trendingMarkets
        : openMarkets.filter((m: any) => m.category === activeCategory);
        ...
        >
          ...
          {userId === ADMIN_ID && (
            <button
              ...
              style={{
                padding: '0.5rem 1.5rem',
                background: view === 'admin' ? '#7c3aed' : '#eee',
            ...
            {leaderboard.map((user: any, index: number) => (
              ...
              >
                ...
                <span style={{ color: '#16a34a', fontWeight: 'bold' }}>
                  💰{' '}
                  {Number(user.balance).toLocaleString('de-DE', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{' '}
                  Dukaten
