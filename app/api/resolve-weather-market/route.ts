    ...
    if (winShares > 0) {
      ...
      if (user) {
        ...
        if (patchRes.ok) {
          ...
          // Fix: "amount"-Feld entfernt (kein Feld im trades-Schema — id, user_id,
          // market_id, type, shares, cost, price_before, price_after, created_at),
          // und cost: -payout → cost: payout (war negativ, jetzt konsistent zu
          // resolve-soccer-market/-crypto-market/-finance-market).
